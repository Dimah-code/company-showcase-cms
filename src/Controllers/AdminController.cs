            ...
            if (cookie != null && cookie["MID"] != null)
            {
                ...
                if (adminInfo != null)
                {
                    ...
                    if (ProfilePic != null && ProfilePic.ContentLength > 0)
                    {
                        ...
                        var path = Path.Combine(Server.MapPath("~/assets/img/AdminProfile"), customFileName);
                        var oldpath = Path.Combine(Server.MapPath("~/assets/img/AdminProfile"), adminInfo.Mpic);
